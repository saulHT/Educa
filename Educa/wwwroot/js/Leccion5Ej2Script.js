...
function initiateGame() {
  ...
  for(let i=0; i<randomDraggableBrands.length; i++) {
    draggableItems.insertAdjacentHTML("beforeend", `
      <img class="draggable" src="/Images/Leccion/Leccion5/Ej2/${randomDraggableBrands[i].iconName}.png" width="48" height="48" draggable="true" style="color: ${randomDraggableBrands[i].color};" id="${randomDraggableBrands[i].iconName}"></img>
    `);
  }
...
function drop(event) {
  ...
  if(isCorrectMatching) {
    ...
    event.target.innerHTML = `<img src="/Images/Leccion/Leccion5/Ej2/${draggableElementBrand}.png" width="48" height="48" style="color: ${draggableElement.style.color};"></img>`;
    rightAudio.play();
    correct++; 
  }
  if(!isCorrectMatching){
  ...
  if(correct===Math.min(totalMatchingPairs, totalDraggableItems)) { // Game Over!!
    ...
    setTimeout(() => {
      link = `/paginas/Leccion5Finish?puntos=${puntos}`;
      winAudio.play();
